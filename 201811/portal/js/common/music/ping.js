...
tcss.prototype = {
	...
	run : function(){
		...
		_cookie.save();
		this.url.unshift((location.protocol == 'https:' ? 'https://pingfore.' : "http://pingfore.") + this.getCookieSetDomain(_curDomain) + "/pingd?");
		...
		n = _params.extParam ? _params.extParam + "|" : "";
	...
	getRefInfo : function(e){
		...
		Tcss.rurl = _refUrl;
		Tcss.rarg = escape(r);
		return c? ("&rdm=" + Tcss.rdm + "&rurl=" + Tcss.rurl + "&rarg=" + Tcss.rarg + "&or=" + c) : ("&rdm=" + Tcss.rdm + "&rurl=" + Tcss.rurl + "&rarg=" + Tcss.rarg);
	},
	getMainEnvInfo : function () {
		...
		try {
			...
			r = u.language? u.language.toLowerCase() : (u.browserLanguage? u.browserLanguage.toLowerCase() : r);
			o = u.javaEnabled() ? 1 : 0;
			i = u.platform;
	...
	getCookieSetDomain : function(e){
		...
		n > -1 && t--;
		r = "qq.com";
		r = t == 1 ? e : (t > 1? e.substring(i[t - 2] + 1) : r);
	...
	watchClick : function(e){
		try {
			var t = !0,
				n = "",
				r;
			r = (window.event ? window.event.srcElement : e.target) || {tagName:""};
			...
			if (t) {
				...
				i.url.unshift((location.protocol == 'https:' ? 'https://pingfore.' : "http://pinghot.") + this.getCookieSetDomain(_curDomain) + "/pingd?");
	...
	getElementPos : function(e){
		...
		if (e.getBoundingClientRect) {
			...
			s = Math.max(document.documentElement.scrollTop, document.body.scrollTop||document.documentElement.scrollTop||window.pageYOffset);
			...
			return {x : (i.left + o - a), y : (i.top + s - u)};
		}
		...
		n = e.parentNode ? e.parentNode : null;
	...
	sendClick : function(){
		_params.hottag && (this.url.push(this.getDomainInfo(!0)), this.url.push("&hottag=" + escape(_params.hottag)), this.url.push("&hotx=9999&hoty=9999"), this.url.push("&rand=" + Math.round(Math.random() * 1e5)), this.url.unshift((location.protocol == 'https:' ? 'https://pingfore.' : "http://pinghot.") + this.getCookieSetDomain(_curDomain) + "/pingd?"), this.sendInfo(this.url.join("")));
	},
	pgvGetArgs : function(){
		this.getDomainInfo();
		var e = [];
...
var pgvSendClick = function(e){
	(new tcss(e)).sendClick();
};

var pgvWatchClick = function(e){
	(new tcss(e)).watchClick(e);
};

var pgvGetArgs = function(e){
	return (new tcss(e)).pgvGetArgs();
}; */


var $ = require('js/common/music/jquery.js'),
	urlPingjs = "//pingjs.qq.com/tcss.ping.js",
	_urlMap = [
		//源地址，目标域名
		//[/http:\/\/music\.qq\.com\/musicbox/i, "http://music.qzone.qq.com"],
		//[/http:\/\/shopcgi\.qqmusic\.qq\.com/i, "http://shop.qqmusic.qq.com"]
	],
	_pathMap = [
		//源路径，目标域名
		//[/\/music\/musicbox_v2_1/i, "http://music.qzone.qq.com"],
		//[/\/music\/musicbox_v3/i, "http://music.qzone.qq.com"]
	];

function setUrlMap(urlMap){
	if($.isArray(urlMap)){
		$.isArray(urlMap[0]) ? $.merge(_urlMap, urlMap) : _urlMap.push(urlMap);
	}
}
function setPathMap(pathMap){
	if($.isArray(pathMap)){
		$.isArray(pathMap[0]) ? $.merge(_pathMap, pathMap) : _pathMap.push(pathMap);
	}
}
/**
 * 发送PV统计信息到点击流系统tcss.boss.com
 */
function pgvMain(param1, param2){
	//var args = [].slice.apply(arguments);
	var n = "";
	param2 ? (n = param2, _ver = "tcsso.3.1.5") : (n = param1, _ver = "tcss.3.1.5");
	try {
		if (_rep != 1) {
			return;
		}
	...
	//require.load(urlPingjs,function(){
	//	pgvMain && pgvMain.apply(w, args);
	//});
}
/**
 * 发送页面按钮点击等统计信息到点击流系统tcss.boss.com
 */
function pgvSendClick(params){
	var args = [].slice.apply(arguments);
	var t = new tcss(args[0]);
	t.sendClick && t.sendClick();
	//require.load(urlPingjs,function(){
	//	pgvSendClick && pgvSendClick.apply(null, args);
	//});
}
function pgvWatchClick(param1, param2){
	var args = [].slice.apply(arguments);
	var n = "";
	param2 ? (n = param2, _ver = "tcsso.3.1.5") : (n = param1, _ver = "tcss.3.1.5");
	try {
		...
		_rep = 2;
		var t = new tcss(n);
		t.watchClick && t.watchClick.apply(t, args);
	} catch (r) {}
	//require.load(urlPingjs,function(){
	//	pgvWatchClick && pgvWatchClick.apply(null, args);
	//});
}
window.pgvWatchClick = pgvWatchClick;
/**
 * 
 */
function pingQQ(path, domain, url){
	if($.type(pgvMain) == "function"){
		var h = url || location.href,
			pvCurDomain = domain || location.host,
			pvCurUrl = path || location.pathname;
	
		//域名映射
		for(var i = 0, l = _urlMap.length; i < l; i++){
			if(_urlMap[i][0].test(h)){
				pvCurDomain = _urlMap[i][1].substr(7);
				break;
			}
		}
		
		for(var i = 0, l = _pathMap.length; i < l; i++){
			if(_pathMap[i][0].test(pvCurUrl)){
				pvCurDomain = _pathMap[i][1].substr(7);
				break;
			}
		}
		
		pgvMain("", {virtualDomain : pvCurDomain, virtualURL : pvCurUrl});
	}
