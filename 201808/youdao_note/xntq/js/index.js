thunder(function() {
    ...
    window.DataChangeRenderPage = {
        ...
        noSuperVipTips: function() {
            var tipsText = Data.item.seat==3?"您还不是超级会员,<br>立即开通，即可获得1元购买机会":Data.item.seat==1?"您还不是超级会员,<br>立即开通，即可享受每月五折购":"您还不是年费超级会员,<br>立即开通，即可享受每月五折购";
            var payid = Data.item.seat==2?"vip2018xntq_cj02":"vip2018xntq_cj01";
            var tpl = ['<div class="mpop_box" id="noSuperVipTips">',
                ...
                '        </div>',
                '        <div class="pop_btn">',
                '            <a href="javascript:;" class="payBtn" payid="'+payid+'" seat="1">开通会员</a>',
                '        </div>',
                '    </div>',
                '    <a class="xl-popbox__btn xl-popbox__btn--primary payBtn" seat="1" href="javascript:;"></a>',
                '</div>'
            ].join("");
        ...
        checkOrderRecorderTips: function(data) {
            ...
            } else {//状态  0：下单完成，支付中  1：取消支付  2：支付成功，充值中  3：充值成功 4：充值失败 5：砍价成功，充值成功 6：砍价成功，充值失败 7：支付成功，砍价中 8：砍价失败
                for (var key in data) {
                    if (data[key].status == 0) {
                        ...
                        statusTxt = "砍价成功，充值失败";
                    }else if(data[key].status == 7){
                        statusTxt = "已支付，剩"+data[key].cut_remain+"人砍价";
                    }else if(data[key].status == 8){
                        statusTxt = "砍价失败";
                    }else if(data[key].status == 9){
                        statusTxt = "砍价成功，充值中";
                    }else{
                        statusTxt = "未知订单状态";
                    }
        ...
        orderReminderTips: function() {
            var tpl = ['<div class="mpop_box" id="orderReminderTips">',
                '    <div class="pop_cont">',
                '        <p class="txt_hd">预约提醒</p>',
                '        <div class="yue_wp">',
                '            <p class="txt_yue">商品上新库存时，我们会短信通知您的哦！</p>',
                '            <div class="yue_ipt">',
                '                <input type="text" placeholder="请输入手机号码" id="phone">',
                '            </div>',
                '            <div class="yue_ipt code">',
                '                <input type="text" placeholder="请输入验证码" id="verifyCode">',
                '                <a href="javascript:;" class="btn_code getVerifyCodeBtnSelector getVerifyCodeBtn">获取验证码</a>',
                '            </div>',
                '        </div>',
                '        <div class="pop_btn">',
                '            <a href="javascript:;" class="submitPreOrderBtn">确定</a>',
                '        </div>',
                '    </div>',
                '    <a href="javascript:msgbox.exit();;" class="pop_close"></a>',
                '</div>'
            ].join("");
            $("#orderReminderTips_cover").remove();
            $("body").append(tpl);
            msgbox.show("orderReminderTips");
        },
        mainPayBtn: function() {
            var txt = "";
        ...
        buyGoodsBtn: function() {
            var classList = "isChildBtn isStopVipBtn noSuperVipTips noYearSuperVipTips payBtn createOrder haveBuyThisMonth haveBuyNoToPay sellOutBtn orderReminderBtn";
            if (user.isChild()) {
                ...
                if (user.isSuperVip()) {
                    if (true) { //乐购专区,1元砍价专区
                        if (Data.tegouOrder.isLast == 1 && Data.isbought == 0) {
                            $(".lgSection .buyGoodsBtnSelector,.kjSection .buyGoodsBtnSelector").removeClass(classList).addClass("haveBuyNoToPay");
                        } else {
                            if (Data.isbought == 0) { //未购买 
                                $(".lgSection .buyGoodsBtnSelector,.kjSection .buyGoodsBtnSelector").removeClass(classList).addClass("createOrder");
                            } else { //已购买 
                                $(".lgSection .buyGoodsBtnSelector,.kjSection .buyGoodsBtnSelector").removeClass(classList).addClass("haveBuyThisMonth");
                            }
                        }
                    }
        ...
        checkHavePaySuccess: function(xlorderid,gid) {
            var _this = this;
            this.data.timeInter = setInterval(function() {
                $.jsonp("https://dynamic-vip-ssl.xunlei.com/iface.php?c=tegouv2&a=getPayStatu&xlorderid=" + xlorderid, function(rs) {
                    if (rs.result == 0) {
                        DataChangeRenderPage.stopCheckOrder();
                        $("#otherBrowser_cover").remove();
                        if (goodsList[gid].seat==3) {
                            DataChangeRenderPage.kanjiaHavePayTips(xlorderid,gid);
                        }else{
                            _this.paySuccessTips();
                        }
                        Data.isbought = 1;
                        DataChangeRenderPage.buyGoodsBtn();
                    }
        ...
        reRenderBtnSellOut: function() {
            var classList = "noSuperVipTips noYearSuperVipTips payBtn createOrder haveBuyThisMonth haveBuyNoToPay sellOutBtn orderReminderBtn";
            for (var key in goodsList) {
                if (goodsList[key].remain == 0) {
                    if (Data.subscribe == 1) { //已预约成功
                        $(".goods" + key + "remain").parents(".item").find(".buyGoodsBtnSelector").removeClass(classList).addClass("sellOutBtn").text("已预约成功").parent().addClass("disabled");
                    } else { //预约提醒
                        $(".goods" + key + "remain").parents(".item").find(".buyGoodsBtnSelector").removeClass(classList).addClass("orderReminderBtn").text("预约提醒").parent().addClass("disabled");
                    }
                } else {
        ...
        loginTips: function() {
            var tpl = ['<div class="mpop_box" id="loginTips">',
                '    <div class="pop_cont">',
                '        <div class="pop_txt">',
                '            <div class="txt_box">',
                '                <p>提前登录，让抢购快人一步</p>',
                ...
                '        </div>',
                '        <div class="pop_btn">',
                '            <a href="javascript:;" class="loginBtn">去登录</a>',
                '        </div>',
                '    </div>',
                '    <a href="javascript:msgbox.exit();" class="pop_close"></a>',
                '</div>'
            ].join("");
            $("#loginTips_cover").remove();
            $("body").append(tpl);
            msgbox.show("loginTips");
        }
    };
    var goodsListObj = {
        init: function() {
            var that = this;
            proInstance = new Promise(function(resolve, reject) {
                that.initGoodsList(resolve);
            });
        },
        data: {
            kjTpl: [],//1元砍价专区
        ...
        initGoodsList: function(resolve) {
            var that = this;
            $.ajax({
        ...
        processData: function(rs) {
            window.goodsList = rs.data.glist;
            window.goodsListArr = [];
            window.orderedGoodsListArr = [];
            kanjiaObj.init();
            if (!haslogin()) {
                goodsListArr = [];
                for (var key in goodsList) {
                ...
                orderedGoodsListArr = Utilities.bubbleSort(goodsListArr);
        ...
        renderGoodsList: function() {
            ...
            for (var key in orderedGoodsListArr) {
                if (orderedGoodsListArr.hasOwnProperty(key)) {
                    if (orderedGoodsListArr[key].seat == 1) {
                        that.data.lgTpl.push(that.getTplSnippet(orderedGoodsListArr[key], "vip2018xntq_cj01"));
                    } else if(orderedGoodsListArr[key].seat == 2){
                        that.data.ryTpl.push(that.getTplSnippet(orderedGoodsListArr[key], "vip2018xntq_cj02"));
                    }else{
                        that.data.kjTpl.push(that.getTplSnippet(orderedGoodsListArr[key], "vip2018xntq_cj02"));
                    }
                }
            }
            $(".kjSection .list").html(that.data.kjTpl.join(""));
            $(".lgSection .list").html(that.data.lgTpl.join(""));
            $(".rySection .list").html(that.data.ryTpl.join(""));
            that.data.kjTpl = [];
            that.data.lgTpl = [];
            that.data.ryTpl = [];
        },
        getTplSnippet: function(item, payid) {
            ...
            var btnText = item.seat==3?"先购买，再砍价":"立即抢购";
            var kjText = item.seat==3?"block":"none";
            var tplSnippet = ['<li class="item">',
                ...
                '    </div>',
                '    <p class="txt_bar" style="display:'+kjText+'">需'+item.cut_num+'人砍价，已砍'+item.cut_count+'件</p>',
                '    <a href="javascript:;" class="btn_buy loginBtn buyGoodsBtnSelector" payid="' + payid + '" gid="' + item.gid + '" seat="' + item.seat + '">'+btnText+'</a>',
                '</li>'
            ].join("");
            return tplSnippet;
        },
    ...
    var kanjiaObj={
        ...
        event:function(){
            ...
            var Events = {
                //initPage
                'document|.helpKanjiaBtn|click': function() {
                    var tipsText = "";
                    if (!haslogin()) {
                        login();
                        return false;
                    }
                    that.kanjiaCommonTips(Data.cutstatus);
                },
            }
            Utilities.initEvents(Events);
        },
        isKanjia:function(){
            var search = location.search.indexOf("shareno");
            var goodsId = Utilities.getUrlParam("gid");
            if (search!==-1) {
            // if (Data.cutstatus>0) {
                this.requestKanjia(goodsId);
            }
        },
        kanjiaCommonTips:function(status){
            var title,descText,tipsType;
            tipsType=Data.cutstatus==5?"pop_img2.png":"pop_img3.png";
            switch(status){
                case 0:
                    title = "砍价失效";
                    descText = "无法完成砍价，你可以自己发起砍价";
                    break;
                case 1:
                    title = "砍价失效";
                    descText = "砍价商品已超时，你可以自己发起砍价";
                    break;
                case 2:
                    title = "砍价失效";
                    descText = "不能给自己砍价哦";
                ...
                case 3:
                    title = "砍价失效";
                    descText = "你已经砍过这件商品啦，你可以自己发起砍价";
                    break;
                case 4:
                    title = "砍价失效";
                    descText = "你的好友已经获得了砍价商品，你可以自己发起砍价";
                    break;
                case 5:
                    title = "砍价成功";
                    descText = "感谢你的热心帮助，还有更多5折话费Q币，你也去看看吧";
                    break;
                case 6:
                    title = "砍价失效";
                    descText = "网络错误，请稍后再试";
            ...
            var tpl = ['<div class="pop_kan" id="kanjiaCommonTips">',
            '    <div class="kan_img">',
            '        <img src="img/'+tipsType+'">',
            '    </div>',
            '    <p class="txt_img">'+title+'</p>',
            '    <p class="txt_kan">'+descText+'</p>',
            '    <a href="javascript:msgbox.exit();" class="btn_kan loginBtn">去逛逛</a>',
            '    <a href="javascript:msgbox.exit();" class="pop_close"></a>',
            '</div>'].join("");
            $("#kanjiaCommonTips_cover").remove();
            $("body").append(tpl);
            msgbox.show("kanjiaCommonTips");
        },
        requestKanjia:function(gid){
            var name = goodsList[gid]?goodsList[gid].name:"迅雷虚拟特权商品";
            var tpl = ['<div class="pop_kan" id="requestKanjia">',
            '    <div class="kan_img">',
            '        <img src="img/pop_img1.png">',
            '        <span class="img_num">1</span>',
            '    </div>',
            '    <p class="txt_img">收到一个砍价请求</p>',
            '    <p class="txt_kan">您的好友正在购买'+name+'，请帮他砍价。砍价后他只需1元即可购买</p>',
            '    <a href="javascript:;" class="btn_kan loginBtn helpKanjiaBtn">帮砍价</a>',
            '    <a href="javascript:msgbox.exit();" class="pop_close"></a>',
            '</div>'].join("");
            $("#requestKanjia_cover").remove();
            $("body").append(tpl);
            msgbox.show("requestKanjia");
        }
    };
    main.onInit(function(data) {
        if (!haslogin()) {
            DataChangeRenderPage.loginTips();
        }
        ...
        var Events = {
            ...
            'document|.orderReminderBtn|click': function() {
                DataChangeRenderPage.orderReminderTips();
            },
            //buyObj
                    ...
                    $.ajax({
                        ...
                        success: function(rs) {
                            ...
                            if (rs.result == 0) {
                                ...
                                // DataChangeRenderPage.checkHavePaySuccess(rs.data.xlorderid,Data.gid);
                            } else {
                                msgbox.showErrorMsg(rs.msg);
                            }
                        },
            ...
            'document|.payCreatedOrderBtn_bak|click': function() {
                ...
                // DataChangeRenderPage.checkHavePaySuccess(Data.tegouOrder.xlorderid,Data.tegouOrder.goodsid);
            },
            ...
            'document|.getVerifyCodeBtn|click': function() {
                ...
                $.ajax({
                    ...
                    success: function(rs) {
                        if (rs.result == 0) {
                            var seconds = 60;
                            alert("验证码，已发送至您的手机。");
                            $(".getVerifyCodeBtnSelector").removeClass("getVerifyCodeBtn");
                            var timer = setInterval(function() {
                                if (seconds > 0) {
                                    $(".getVerifyCodeBtnSelector").text(--seconds + "s");
                                } else {
                                    $(".getVerifyCodeBtnSelector").addClass("getVerifyCodeBtn").text("获取验证码");
                                    clearInterval(timer);
                                }
                            }, 1000);
                        } else {
            ...
            'document|.submitPreOrderBtn|click': function() {
                if (!(/^\d{11}$/.test($("#phone").val()))) {
                    alert("输入的手机号不正确");
                    $("#phone").focus();
                    return false;
                ...
                $.ajax({
                    ...
                    success: function(rs) {
                        if (rs.result == 0) {
                            Data.subscribe = 1
                            DataChangeRenderPage.reRenderBtnSellOut();
                            msgbox.showErrorMsg("预约成功");
                        } else {
                            alert(rs.msg);
                        }
                    },
    ...
    main.onLoginInit(function(data) {
        var isSuperVip = data.userInfo.isvip==1&&data.userInfo.vas_type==5?true:false;
        var vipLevel = data.userInfo.level||0;
        Data.vip8price=data.vip8price;
        var loginObj = {
            init: function() {
                this.initData();
                this.vip8();
            ...
            vip8: function() {
                if (user.isSuperVip() && (user.level() >= 8)) {
                    this.isExistCookie();
                }
            },
            isExistCookie: function() {
                var curDate = new Date();
                var dateStr = curDate.getFullYear() + "_" + (curDate.getMonth() + 1) + "_" + curDate.getDate() + "_" + getCookie('userid');
                if (Utilities.getCookie("xntq_wap") != dateStr) {
                    this.youAreSvip8();
                    Utilities.setCookie("xntq_wap", dateStr, 24);
                }
            },
            youAreSvip8: function() {
                var tpl = ['<div class="mpop_box" id="youAreSvip8">',
                    '    <div class="pop_cont">',
                    '        <div class="pop_txt">',
                    '            <div class="txt_box">',
                    '                <p>尊敬的SVIP' + user.level() + '会员，恭喜您获得全场'+Data.vip8price/100+'元钱<br>购买虚拟商品的机会。</p>',
                    '            </div>',
                    '        </div>',
                    '        <div class="pop_btn">',
                    '            <a href="javascript:msgbox.exit();">去逛逛</a>',
                    '        </div>',
                    '    </div>',
                    '    <a href="javascript:msgbox.exit();;" class="pop_close"></a>',
                    '</div>'
                ].join("");
                $("#youAreSvip8_cover").remove();
                $("body").append(tpl);
                msgbox.show("youAreSvip8");
            },
            initData: function() {
                ...
                Data.tegouOrder=data.tegouOrder;
                Data.userInfo=data.userInfo;
                Data.user_type=data.user_type;
                Data.vip8price=data.vip8price;
                Data.cutstatus=data.cutstatus;
            },
        ...
        proInstance.then(function(val) {
            if (isSuperVip && vipLevel >= 8) {
                $(".kjSection").hide();
                goodsListArr = [];
                for (var key in goodsList) {
                    if (goodsList.hasOwnProperty(key)) {
                        goodsList[key].sj_price = Data.vip8price/100;
                        goodsListArr.push(goodsList[key]);
                    }
                }
                orderedGoodsListArr = Utilities.bubbleSort(goodsListArr);
                goodsListObj.renderGoodsList();
            } else {
                goodsListArr = [];
                for (var key in goodsList) {
                    if (goodsList.hasOwnProperty(key)) {
